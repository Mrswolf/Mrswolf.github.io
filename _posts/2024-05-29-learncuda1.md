...
上一篇[post]({% post_url 2024-05-18-learncuda0 %})实现了一个基本的cuda程序，本篇记录如何实现矩阵乘法并加速。<!--more-->主要内容是对[blog](https://siboehm.com/articles/22/CUDA-MMM)内容的学习和复现，目的是逐步逼近cuBLAS实现的单精度矩阵乘法：
$$
...
理解memory coalescing的另一个关键是GPU是SIMT的架构，同一时刻，同一个warp中的线程执行的是相同的指令。例如，之前的kernel访问`A[i*K+k]`，可以认为同一时间点，一个warp中的32个threads都在相同的`k`索引处访问数组`A`，但是它们具有不同的`i`索引。虽然不同threads寄存器的值不同（因此访问的是A的不同位置），但是它们遵循的加载指令是一致的。
